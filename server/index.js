...
const storeAccessMiddleware = async (req, res, next) => {
  try {
    const user = await prisma.user.findUnique({
      where: { id: req.userId },
      select: { role: true, storeAccess: true }
    });
    // Admins/super-admins bypass store access checks
    if (['ADMIN', 'SUPER_ADMIN'].includes(user.role)) return next();

    const access = user.storeAccess || [1];
    if (!access.includes(req.storeId)) {
      return res.status(403).json({ error: `You don't have access to Store ${req.storeId}` });
    }
    next();
  } catch (err) {
...
app.get('/api/players', authMiddleware, storeAccessMiddleware, async (req, res) => {
  try {
...
app.post('/api/create-new-player', authMiddleware, storeAccessMiddleware, async (req, res) => {
  try {
    const {
...
app.patch('/api/players/:id', authMiddleware, async (req, res) => {
  ...
  'referredById', 'friendIds',
      ...
      if (existing) {
        ...
        broadcastTaskUpdate('edit_request_updated', { request: updated, storeId: req.storeId }, req.storeId);
        return res.json({
          pending: true,
          message: 'Edit request updated — awaiting admin approval.',
          data: updated,
        });
      }

      const request = await prisma.playerEditRequest.create({
        data: {
          storeId: req.storeId,
          playerId: id,
      ...
      broadcastTaskUpdate('edit_request_created', { request, storeId: req.storeId }, req.storeId);
      return res.status(201).json({
        pending: true,
        message: `Edit submitted for ${request.player.name} — waiting for admin approval.`,
        data: request,
      });
    }
...
// ── GET /api/player-edit-requests ─────────────────────────────────────────
// Admin: list all pending (or all) edit requests
app.get('/api/player-edit-requests', authMiddleware, adminMiddleware, async (req, res) => {
  try {
...
// ── POST /api/player-edit-requests/:id/approve ────────────────────────────
app.post('/api/player-edit-requests/:id/approve', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const reqId = parseInt(req.params.id);
    const { note } = req.body;

    const editReq = await prisma.playerEditRequest.findUnique({
      where: { id: reqId },
      include: {
    ...
    if (editReq.status !== 'PENDING') {
      return res.status(400).json({ error: `Request is already ${editReq.status.toLowerCase()}` });
    }
  ...
  if (changes.referredById !== undefined) {
    ...
    const [updatedPlayer, updatedRequest] = await prisma.$transaction([
      prisma.user.update({
        where: { id: editReq.playerId },
        data: updateData,
      }),
      prisma.playerEditRequest.update({
        where: { id: reqId },
        data: {
          status: 'APPROVED',
...
// ── totalBonusEarned adjustment ──────────────────────────────
  ...
  if (changes.friendIds !== undefined && Array.isArray(changes.friendIds)) {
    ...
    res.json({
      success: true,
      message: `Edits for ${editReq.player.name} approved and applied.`,
      data: { player: updatedPlayer, request: updatedRequest },
    });
  } catch (err) {
...
// ── POST /api/player-edit-requests/:id/reject ─────────────────────────────
app.post('/api/player-edit-requests/:id/reject', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const reqId = parseInt(req.params.id);
    const { note } = req.body;
    ...
    const updated = await prisma.playerEditRequest.update({
      where: { id: reqId },
      data: { status: 'REJECTED', reviewedBy: req.userId, reviewNote: note || null },
    });
    ...
    res.json({
      success: true,
      message: `Edit request for ${editReq.player.name} rejected.`,
      data: updated,
    });
  } catch (err) {
...
// ─── PATCH NOTES for server.js ───────────────────────────────────────────────
//
// BUG 1 (Critical): referralBonus.create referenced `depositTx.id` BEFORE
//   prisma.$transaction(ops) ran — depositTx was always undefined, so the
//   ReferralBonus record was never written to DB and eligible-bonuses always
//   returned empty.
//
//   FIX: move the referralBonus.create call to AFTER `results` is destructured.
//
// BUG 2 (Runtime error): the response object referenced `referralAmt` and
//   `referrer` which are from commented-out code and are undefined at runtime,
//   causing a crash on every deposit.
//
//   FIX: remove those undefined references from the response.
//
// ─── Replace the entire POST /api/transactions/deposit handler with this ─────

app.post('/api/transactions/deposit', authMiddleware, storeAccessMiddleware, async (req, res) => {
  try {
    const {
    ...
    if (bonusMatch) {
      ...
      const existingMatchBonus = await prisma.transaction.findFirst({
        where: { userId: parseInt(playerId), type: 'BONUS', status: 'COMPLETED', description: { contains: 'Match Bonus' }, createdAt: { gte: todayStart, lte: todayEnd } },
      });
      if (existingMatchBonus) return res.status(400).json({ error: 'Match bonus already used today for this player.' });
    }

    // ── Referral eligibility guard ────────────────────────────────────────────
    if (bonusReferral) {
      if (!player.referredBy)
        return res.status(400).json({ error: 'Player has no referrer — referral bonus cannot be recorded.' });

      const existingRecord = await prisma.referralBonus.findFirst({
        where: { referrerId: player.referredBy, referredId: parseInt(playerId) },
      });
      if (existingRecord)
        return res.status(400).json({
          error: 'Referral bonus eligibility already recorded for this player.',
    ...
    if (!wallet) return res.status(404).json({ error: 'Wallet not found' });
    if (!wallet.isLive) return res.status(400).json({ error: `Wallet "${wallet.name}" is currently offline.` });
    ...
    ops.push(prisma.transaction.create({    // results[2] → depositTx
      data: {
        userId: parseInt(playerId), type: 'DEPOSIT',
  ...
  console.error(`⚠️  [MW Sync] Deposit sync threw for "${player.username}":`, err.message);
    ...
    res.status(201).json({
      ...
      transaction: {
        ...
        referralEligibility: (bonusReferral && player.referredBy)
          ? { referrerId: player.referredBy, bonusAmount: parseFloat((depositAmt / 2).toFixed(2)) }
          : null,
      },
...
// GET /api/players/:id/eligible-bonuses
app.get('/api/players/:id/eligible-bonuses', authMiddleware, async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    if (isNaN(id)) return res.status(400).json({ error: 'Invalid player ID' });

    // Records where this player is the REFERRED party (B) and their side is unclaimed
    const asReferred = await prisma.referralBonus.findMany({
      where: { referredId: id, referredClaimed: false },
      include: {
        referrer: { select: { id: true, name: true, username: true } },
      },
      orderBy: { createdAt: 'desc' },
    ...
    // Records where this player is the REFERRER (A) and their side is unclaimed
    const asReferrer = await prisma.referralBonus.findMany({
      where: { referrerId: id, referrerClaimed: false },
      include: {
        referred: { select: { id: true, name: true, username: true } },
      },
      orderBy: { createdAt: 'desc' },
    });
    ...
    res.json({ data: eligible, total: eligible.length });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

// POST /api/referral-bonuses/:id/claim
...
//     const rb = await prisma.referralBonus.findUnique({
//       where: { id: rbId },
//       include: {
//         referrer: { select: { id: true, name: true, balance: true } },
//         referred: { select: { id: true, name: true, balance: true } },
//       }
//     });
//     if (!rb) return res.status(404).json({ error: 'Referral bonus record not found' });

//     const alreadyClaimed = side === 'referrer' ? rb.referrerClaimed : rb.referredClaimed;
//     if (alreadyClaimed) return res.status(400).json({ error: `${side} side already claimed` });

//     const game = await prisma.game.findUnique({ where: { id: gameId } });
//     if (!game) return res.status(404).json({ error: 'Game not found' });

//     const bonusAmt = parseFloat(rb.bonusAmount);
...
//           : { referredClaimed: true, referredClaimedAt: new Date(), gameId },
//       }),
//     ]);
...
//       data: { transactionId: tx.id, recipient: { id: recipient.id, name: recipient.name }, bonusAmount: bonusAmt },
//     });
//   } catch (err) {
//     res.status(500).json({ error: 'Failed to claim referral bonus: ' + err.message });
//   }
// });
...
app.post('/api/referral-bonuses/:id/claim', authMiddleware, async (req, res) => {
  try {
    const rbId = parseInt(req.params.id);
    const { gameId, notes, amount: customAmount, grantBoth = false } = req.body;
    ...
    if (!rb) return res.status(404).json({ error: 'Referral bonus record not found' });
    if (rb.referrerClaimed) return res.status(400).json({ error: 'Referrer side already claimed' });

    const game = await prisma.game.findUnique({ where: { id: gameId } });
    if (!game) return res.status(404).json({ error: 'Game not found' });

    // Use custom amount if provided, otherwise fall back to stored bonusAmount
    const bonusAmt = customAmount && parseFloat(customAmount) > 0
      ? parseFloat(parseFloat(customAmount).toFixed(2))
      : parseFloat(rb.bonusAmount);

    const totalDeduction = grantBoth ? bonusAmt * 2 : bonusAmt;

    if (game.pointStock < totalDeduction)
      return res.status(400).json({
        error: `Insufficient game stock. Need ${totalDeduction.toFixed(2)} pts, ${game.name} has ${game.pointStock.toFixed(2)}.`
      });

    const newStock = game.pointStock - totalDeduction;
    const newGameStatus = newStock <= 0 ? 'DEFICIT' : newStock <= 500 ? 'LOW_STOCK' : 'HEALTHY';

    const referrer = rb.referrer;
    const referred = rb.referred;

    const referrerBalBefore = parseFloat(referrer.balance);
    const referrerBalAfter = referrerBalBefore + bonusAmt;
    const referrerDesc = `Referral Bonus from ${game.name} — ${referred.name} deposited $${parseFloat(rb.depositAmount).toFixed(2)}`;

    const ops = [
      // 1. Update game stock
      prisma.game.update({ where: { id: gameId }, data: { pointStock: newStock, status: newGameStatus } }),

      // 2. Referrer transaction (always granted)
      prisma.transaction.create({
        data: {
          userId: referrer.id, type: 'BONUS', amount: bonusAmt, status: 'COMPLETED',
          description: referrerDesc, gameId,
          notes: `balanceBefore:${referrerBalBefore}|balanceAfter:${referrerBalAfter}|gameStockBefore:${game.pointStock.toFixed(2)}|gameStockAfter:${newStock.toFixed(2)}|${notes || ''}`,
        }
      }),

      // 3. Referrer bonus record
      prisma.bonus.create({
        data: { userId: referrer.id, type: 'REFERRAL', amount: bonusAmt, description: referrerDesc, claimed: true, claimedAt: new Date() }
      }),
    ...
    if (grantBoth) {
      const referredBalBefore = parseFloat(referred.balance);
      const referredBalAfter = referredBalBefore + bonusAmt;
      const referredDesc = `Referral Bonus from ${game.name} — referred by ${referrer.name}`;

      ops.push(
        prisma.transaction.create({
          data: {
            userId: referred.id, type: 'BONUS', amount: bonusAmt, status: 'COMPLETED',
            description: referredDesc, gameId,
            notes: `balanceBefore:${referredBalBefore}|balanceAfter:${referredBalAfter}|gameStockBefore:${game.pointStock.toFixed(2)}|gameStockAfter:${newStock.toFixed(2)}|${notes || ''}`,
          }
        }),
        prisma.bonus.create({
          data: { userId: referred.id, type: 'REFERRAL', amount: bonusAmt, description: referredDesc, claimed: true, claimedAt: new Date() }
        }),
      );
    }
    ...
    res.json({
      success: true,
      message: grantBoth
        ? `$${bonusAmt.toFixed(2)} Referral Bonus granted to both ${referrer.name} (referrer) and ${referred.name} (referred) from ${game.name}.`
        : `$${bonusAmt.toFixed(2)} Referral Bonus granted to ${referrer.name} (referrer) from ${game.name}.`,
      data: {
        transactionId: referrerTx.id,
        referrer: { id: referrer.id, name: referrer.name },
        referred: grantBoth ? { id: referred.id, name: referred.name } : null,
        bonusAmount: bonusAmt, grantBoth,
      },
    });
  } catch (err) {
    res.status(500).json({ error: 'Failed to claim referral bonus: ' + err.message });
...
// GET /api/referral-bonuses — full ledger for admin
app.get('/api/referral-bonuses', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const records = await prisma.referralBonus.findMany({
      include: {
        referrer: { select: { id: true, name: true, username: true } },
        referred: { select: { id: true, name: true, username: true } },
      },
...
app.post('/api/transactions/cashout', authMiddleware, async (req, res) => {
  try {
    ...
    // ✅ FIXED:
    // ✅ Only 1 item — destructure just one variable
...
app.patch('/api/transactions/:transactionId/approve', authMiddleware, async (req, res) => {
  try {
    const transactionId = parseInt(req.params.transactionId);
...
app.post('/api/transactions/:transactionId/partial-payment', authMiddleware, async (req, res) => {
  try {
    const transactionId = parseInt(req.params.transactionId);
    if (isNaN(transactionId)) return res.status(400).json({ error: 'Invalid transaction ID' });
...
app.get('/api/players/:id/pending-bonuses', authMiddleware, async (req, res) => {
  try {
    ...
    const [milestones, referralWeekly] = await Promise.all([
      // This player's own unclaimed milestone bonuses
      prisma.depositMilestoneBonus.findMany({
        where: { playerId: id, claimed: false },
        orderBy: { createdAt: 'desc' },
      }),
      // Unclaimed weekly referral bonuses WHERE this player is the referrer
      prisma.referralWeeklyBonus.findMany({
        where: { referrerId: id, claimed: false },
        include: { referred: { select: { id: true, name: true, username: true } } },
        orderBy: { createdAt: 'desc' },
      }),
    ]);

    const totalAmount =
      milestones.reduce((s, m) => s + parseFloat(m.bonusAmount), 0) +
      referralWeekly.reduce((s, r) => s + parseFloat(r.bonusAmount), 0);
    ...
    res.json({
      data: {
        ...
        referralWeekly: referralWeekly.map(r => ({
          id: r.id,
          type: 'referral_weekly',
          weekOf: r.weekOf,
          totalDeposits: parseFloat(r.totalDeposits),
          bonusAmount: parseFloat(r.bonusAmount),
          referredPlayer: r.referred,
          claimed: r.claimed,
          createdAt: r.createdAt,
        })),
        totalPending: milestones.length + referralWeekly.length,
...
// ── POST /api/milestone-bonuses/:id/claim ─────────────────────────
// Staff claims a milestone bonus for the player. Deducts from game stock,
// creates a BONUS transaction, and increments player balance.
app.post('/api/milestone-bonuses/:id/claim', authMiddleware, async (req, res) => {
  try {
    ...
    const bonus = await prisma.depositMilestoneBonus.findUnique({
      where: { id: bonusId },
      include: { player: { select: { id: true, name: true, balance: true } } },
    });
    if (!bonus) return res.status(404).json({ error: 'Milestone bonus not found' });
    if (bonus.claimed) return res.status(400).json({ error: 'Bonus already claimed' });
    ...
    const newStock = game.pointStock - bonusAmt;
    const newStatus = newStock <= 0 ? 'DEFICIT' : newStock <= 500 ? 'LOW_STOCK' : 'HEALTHY';
    const balBefore = parseFloat(bonus.player.balance);
    const balAfter = balBefore + bonusAmt;
    const desc = `Milestone Bonus from ${game.name} — $${bonus.milestone} daily deposit milestone (${bonus.date})`;

    const [, , tx] = await prisma.$transaction([
      prisma.depositMilestoneBonus.update({
        where: { id: bonusId },
        data: { claimed: true, claimedAt: new Date(), gameId },
      }),
      prisma.game.update({
        where: { id: gameId },
        data: { pointStock: newStock, status: newStatus },
      ...
      prisma.transaction.create({
        data: {
          ...
          description: desc,
          gameId,
          notes: `balanceBefore:${balBefore}|balanceAfter:${balAfter}|` +
            `gameStockBefore:${game.pointStock.toFixed(2)}|gameStockAfter:${newStock.toFixed(2)}|` +
            `milestoneId:${bonusId}|${notes || ''}`,
        },
      }),
      prisma.bonus.create({
        data: {
      ...
      // Balance = deposit - cashout; bonuses only add when explicitly redeemed here
      prisma.user.update({
    ...
    res.json({
      success: true,
      message: `$${bonusAmt.toFixed(2)} Milestone Bonus granted to ${bonus.player.name} from ${game.name}!`,
      data: { transactionId: tx.id, bonusAmount: bonusAmt, newBalance: balAfter },
    });
  } catch (err) {
    res.status(500).json({ error: 'Failed to claim milestone bonus: ' + err.message });
  }
});

// ── POST /api/referral-weekly-bonuses/:id/claim ───────────────────
// Staff claims a weekly referral bonus for the referrer.
app.post('/api/referral-weekly-bonuses/:id/claim', authMiddleware, async (req, res) => {
  try {
    const bonusId = parseInt(req.params.id);
    const { gameId, notes } = req.body;
    if (!gameId) return res.status(400).json({ error: 'gameId is required' });

    const bonus = await prisma.referralWeeklyBonus.findUnique({
      where: { id: bonusId },
      include: {
        referrer: { select: { id: true, name: true, balance: true } },
    ...
    if (!bonus) return res.status(404).json({ error: 'Referral weekly bonus not found' });
    if (bonus.claimed) return res.status(400).json({ error: 'Bonus already claimed' });
    ...
    const newStock = game.pointStock - bonusAmt;
    const newStatus = newStock <= 0 ? 'DEFICIT' : newStock <= 500 ? 'LOW_STOCK' : 'HEALTHY';
    const balBefore = parseFloat(bonus.referrer.balance);
    const balAfter = balBefore + bonusAmt;
    const desc =
      `Referral Weekly Bonus from ${game.name} — ` +
      `10% of ${bonus.referred.name}'s $${parseFloat(bonus.totalDeposits).toFixed(2)} deposits (week of ${bonus.weekOf})`;

    const [, , tx] = await prisma.$transaction([
      prisma.referralWeeklyBonus.update({
        where: { id: bonusId },
        data: { claimed: true, claimedAt: new Date(), gameId },
      }),
      prisma.game.update({
    ...
    res.json({
      success: true,
      message: `$${bonusAmt.toFixed(2)} Referral Weekly Bonus granted to ${bonus.referrer.name} from ${game.name}!`,
      data: { transactionId: tx.id, bonusAmount: bonusAmt },
    });
  } catch (err) {
    res.status(500).json({ error: 'Failed to claim referral weekly bonus: ' + err.message });
  }
});

// ── GET /api/pending-bonuses/all ─────────────────────────────────
// Admin view: all unclaimed pending bonuses across all players.
app.get('/api/pending-bonuses/all', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const [milestones, referralWeekly] = await Promise.all([
      prisma.depositMilestoneBonus.findMany({
        where: { claimed: false },
    ...
    res.json({
      data: {
        milestones: milestones.map(m => ({ ...m, bonusAmount: parseFloat(m.bonusAmount) })),
        referralWeekly: referralWeekly.map(r => ({
          ...r,
          totalDeposits: parseFloat(r.totalDeposits),
          bonusAmount: parseFloat(r.bonusAmount),
...
app.post('/api/shifts/start', authMiddleware, async (req, res) => {
  try {
    const user = await prisma.user.findUnique({ where: { id: req.userId }, select: { teamSlot: true, name: true } });
    const teamRole = `SLOT_${user.teamSlot}`;
    if (!teamRole) return res.status(400).json({ error: 'teamRole is required' });
    ...
    notify('SHIFT_START', { memberName: user.name, teamRole, shiftId: shift.id });
    res.status(201).json({ data: shift, message: 'Shift started' });
  } catch (err) {
    res.status(500).json({ error: 'Failed to start shift: ' + err.message });
  }
...
app.get('/api/shifts/my-status', authMiddleware, async (req, res) => {
  try {
    const user = await prisma.user.findUnique({
      where: { id: req.userId },
      select: { role: true, storeAccess: true }
  ...
  where: {
    ...
    res.json({ data: results, storeAccess: user.storeAccess || [1] });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
async function enrichShift(shift) {
  ...
  // Get storeId from the shift's team
  const team = await prisma.team.findUnique({
    where: { id: shift.teamId },
    select: { storeId: true }
  });
  const storeId = team?.storeId || 1;

  // Get player IDs for THIS store only
  const storeUserIds = await prisma.user
    .findMany({ where: { role: 'PLAYER', storeId }, select: { id: true } })
    .then(us => us.map(u => u.id));
  ...
  return {
    ...
    stats: {
      ...
      netProfit: f2(totalDeposits - totalCashouts),
...
//     });
    ...
    res.json({
      ...
      summary: {
        ...
        netProfit: f2(dayDeposits - dayCashouts),
...
app.patch('/api/users/:id/store-access', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const { storeAccess } = req.body; // [1], [2], or [1, 2]
    if (!Array.isArray(storeAccess)) return res.status(400).json({ error: 'storeAccess must be an array' });

    const updated = await prisma.user.update({
      where: { id },
      data: { storeAccess },
      select: { id: true, name: true, role: true, storeAccess: true }
    });
    res.json({ data: updated });
  } catch (err) {
    res.status(500).json({ error: err.message });
