...
async function main() {
    ...
    //     }
    // });
    // ── Store 1 team (existing) ──────────────────────────────────
    const team1 = await prisma.user.create({
        data: {
            username: 'team1', password: teamPassword,
            name: 'T1', email: 'team1@oceanbets.com',
            role: 'TEAM1', storeId: 1, storeAccess: [1],
        }
    });
    const team2 = await prisma.user.create({
        data: {
            username: 'team2', password: teamPassword,
            name: 'T2', email: 'team2@oceanbets.com',
            role: 'TEAM2', storeId: 1, storeAccess: [1],
        }
    });
    const team3 = await prisma.user.create({
        data: {
            username: 'team3', password: teamPassword,
            name: 'T3', email: 'team3@oceanbets.com',
            role: 'TEAM3', storeId: 1, storeAccess: [1],
        }
    });
    const team4 = await prisma.user.create({
        data: {
            username: 'team4', password: teamPassword,
            name: 'T4', email: 'team4@oceanbets.com',
            role: 'TEAM4', storeId: 1, storeAccess: [1],
        }
    });

    // ── Store 2 team (new) ───────────────────────────────────────
    const team5 = await prisma.user.create({
        data: {
            username: 'team5', password: teamPassword,
            name: 'T5', email: 'team5@oceanbets.com',
            role: 'TEAM1', storeId: 2, storeAccess: [2],
        }
    });
    const team6 = await prisma.user.create({
        data: {
            username: 'team6', password: teamPassword,
            name: 'T6', email: 'team6@oceanbets.com',
            role: 'TEAM2', storeId: 2, storeAccess: [2],
        }
    });
    const team7 = await prisma.user.create({
        data: {
            username: 'team7', password: teamPassword,
            name: 'T7', email: 'team7@oceanbets.com',
            role: 'TEAM3', storeId: 2, storeAccess: [2],
        }
    });
    const team8 = await prisma.user.create({
        data: {
            username: 'team8', password: teamPassword,
            name: 'T8', email: 'team8@oceanbets.com',
            role: 'TEAM4', storeId: 2, storeAccess: [2],
        }
    });

    // ── Cross-store member (Store 1 + 2) ────────────────────────
    const teamCross = await prisma.user.create({
        data: {
            username: 'team_cross', password: teamPassword,
            name: 'Cross Store Member', email: 'cross@oceanbets.com',
            role: 'TEAM1',
            storeId: 1,          // home store
            storeAccess: [1, 2], // ← can switch to both
        }
    });

    // ── Admin: always has access to everything (storeAccess not checked) ─
    const admin = await prisma.user.create({
        data: {
            username: 'admin', password: adminPassword,
            name: 'Admin User', email: 'admin@oceanbets.com',
            role: 'ADMIN',
            storeId: 1,
            storeAccess: [1],   // doesn't matter — adminMiddleware bypasses check
        }
    });
    ...
    for (const game of games) {
        const upserted = await prisma.game.upsert({
            where: { storeId_slug: { storeId: game.storeId, slug: game.slug } },
            update: { pointStock: game.pointStock, status: game.status },
            create: {
                name: game.name,
                slug: game.slug,
                pointStock: game.pointStock,
                status: game.status,
